...
**Fetching is decoupled from serving.** A user opening the app is *never* the trigger for an
Open-Meteo call. A GitHub Action (`.github/workflows/refresh-snapshot.yml`) runs hourly, calls every
live source via `snapshot.py --refresh`, and commits `data/snapshot.json`; the app only ever *reads*
that file. If a scheduled fetch fails nobody is watching — it retries next hour while the app keeps
serving the last good snapshot. This is what makes a rate limit on a user's page load structurally
impossible rather than merely unlikely.

Layered fallback, in order: **committed snapshot** → (only if the snapshot is >12 h old, i.e. the
scheduler itself looks broken) **live fetch** → **last-good disk cache** (up to 7 days) → a plain
message. The UI always states which one it used and how old the numbers are; staleness is labelled,
never hidden. There is **no `st.stop()` anywhere** — a bad day degrades the forecast panel and leaves
the evaluation and method tabs fully rendered.

Per-source merge: each source refreshes independently, so a GloFAS outage keeps the previous GloFAS
value and never costs you the weather. The refresh rewrites `snapshot.json` only when the *values*
move (Open-Meteo serves one model run for hours), so an hourly job produces ~4–8 commits/day, not 24.
...
deterministic forecast + USGS + GloFAS — **never** the archive or historical-forecast
...
> **Snapshot size.** The ensemble is stored **daily-aggregated**, which is exactly what the app
> consumes — `_ensemble_frame` collapses ~192 hourly steps × 31 members to 8 daily rows anyway. All
> seven sources, three ensemble systems included, fit in **23 KB**.

> **Tradeoff.** Committing to the deployed branch means Streamlit Community Cloud redeploys on each
> snapshot commit. If that becomes disruptive, push the snapshot to a separate `snapshot-data`
> branch and have `snapshot.load()` read it from `raw.githubusercontent.com` — GitHub's CDN is a far
> more reliable dependency than a rate-limited weather API, though it does put one network call back
> on the render path.
...
live_data.py         live pipeline: deterministic + ensemble forecast, USGS, GloFAS;
                     build 7 rows, predict_week (with ensemble bands). Stable cache
                     keys + last-good fallback + failure cooldown.
snapshot.py          out-of-band snapshot: --refresh writes data/snapshot.json (CI),
                     and the accessors the app reads instead of calling any API
.github/workflows/refresh-snapshot.yml
                     hourly scheduled refresh; commits the snapshot back to the repo
...
venv\Scripts\python snapshot.py --refresh                    # fetch all live sources -> data/snapshot.json
venv\Scripts\python snapshot.py                              # show what's in the snapshot + per-source age
...
**Commit `data/snapshot.json`** — the app reads it on first boot, so without it a cold container
has to fall back to a live call (exactly the dependency we removed). The workflow needs
*Settings → Actions → General → Workflow permissions = Read and write*. Note that GitHub disables
scheduled workflows after 60 days of repo inactivity; the job's own commits count as activity, so a
healthy schedule keeps itself alive.
