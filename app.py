...
# Every live getter now goes through snapshot.py, which reads the scheduled
# snapshot FIRST and only touches the network as a self-heal. A user opening the
# page is no longer the trigger for an Open-Meteo call, so a rate limit on their
# page load is not a thing that can happen. Each returns (value, meta) where meta
# records where the number came from and how old it is -> the freshness chip.
@st.cache_data(ttl=900, show_spinner=False)
def get_weather(lat, lon):
    w, meta = snap.weather(lat, lon)
    return w, w.attrs.get("fetched_at"), meta


@st.cache_data(ttl=900, show_spinner=False)
def get_river(gauge_id):
    return snap.river(gauge_id)


@st.cache_data(ttl=900, show_spinner=False)
def get_ensemble(lat, lon, model):
    return snap.ensemble(lat, lon, model)


@st.cache_data(ttl=900, show_spinner=False)
def get_glofas(lat, lon):
    return snap.glofas(lat, lon)


@st.cache_data(ttl=900, show_spinner=False)
def get_river_history(gauge_id, days=35):
    """Recent USGS observed daily discharge for the context sparkline.
    USGS 'daily' collection -- a live context call, not an Open-Meteo eval endpoint."""
    return snap.river_history(gauge_id, days)
...
def freshness_chip(metas: dict) -> None:
    """One honest line about how old the numbers are.

    Policy: never hide staleness, never make it the headline. A forecast from a
    few hours ago is the normal case (the scheduler runs hourly); a page that
    refuses to render is not an acceptable alternative to a slightly old one."""
    wm = metas.get("weather") or {}
    src, age = wm.get("source"), wm.get("age_seconds")
    when = _age_str(age)
    degraded = [k for k, m in metas.items() if (m or {}).get("source") == "unavailable"]

    if src in ("snapshot", "live"):
        stale = age is not None and age > snap.SELF_HEAL_AFTER_SECONDS
        icon = "🟡" if stale else "🟢"
        msg = f"{icon} Forecast data updated **{when}**"
        msg += " · refreshed on a schedule, not on your page load" if src == "snapshot" else " · fetched live"
        if stale:
            msg += " — the scheduled refresh looks overdue, so this may be behind"
    else:  # stale-cache: the live path fell back to its last good copy
        msg = (f"🟡 Showing the forecast from **{when}** — live data is temporarily "
               f"unavailable, so this is the last good copy")
    if degraded:
        msg += f" · unavailable right now: {', '.join(degraded)}"
    st.caption(msg)
...
# Flood thresholds come from the trained bundle, never from a live call -- hoisted
# here so the evaluation tabs below do not depend on anything the main tab fetches.
thr_cfs = bundles["A"]["flood_threshold_cfs"]
thr_mm = bundles["A"]["flood_threshold_mm_day"]

# ===========================================================================
# MAIN
# ===========================================================================
with tab_main:
    # Snapshot-first: this normally resolves from data/snapshot.json with no network
    # at all. Everything that can fail raises LiveDataError and lands in the single
    # handler at the bottom of this block -- there is no st.stop() anywhere now, so
    # a bad day here can never blank the evaluation and method tabs the way it did
    # before (they are populated further down the same script run).
    try:
        weather, fetched_at, w_meta = get_weather(basin["lat"], basin["lon"])
        river, r_meta = get_river(basin["id"])
        ...
        ensemble, e_meta = get_ensemble(basin["lat"], basin["lon"], ens_model)
        if ensemble is None and ens_model != "gfs_seamless":
            ...
            ensemble, e_meta = get_ensemble(basin["lat"], basin["lon"], "gfs_seamless")
        glofas, g_meta = get_glofas(basin["lat"], basin["lon"])
        freshness_chip({"weather": w_meta, "ensemble band": e_meta,
                        "GloFAS": g_meta, "river level": r_meta})
        ...
        except Exception as e:  # -> the single handler below; never a dead page
            raise ld.LiveDataError(f"could not score today's forecast: {e}") from e
        ...
        with cc1:
            ...
            rh, _rh_meta = get_river_history(basin["id"])
        ...
        st.caption(f"Forecast fetched: {res.get('fetched_at','?')} "
                   f"({_age_str(w_meta.get('age_seconds'))}, via {w_meta.get('source')}) · "
                   f"Weather/ensemble: Open-Meteo ({res.get('ensemble_model') or 'deterministic'}) · "
                   f"GloFAS: Open-Meteo Flood · River: modern USGS API · "
                   f"Refreshed on a schedule by .github/workflows/refresh-snapshot.yml, "
                   f"never on your page load")
        ...
        # Reaching here needs ALL of: no committed snapshot, no last-good disk
        # cache (7-day window), and no network. Say so plainly, without the raw
        # URL, and point at the tabs that genuinely DO still render below.
        st.warning("⚠️ Today's forecast isn't available yet — there's no scheduled snapshot to "
                   "read and live weather can't be reached from here right now. The forecast "
                   "refreshes on a schedule, so this usually clears on its own; try again "
                   "shortly. **The model, its evaluation and the method write-up are all "
                   "available in the other two tabs.**")
