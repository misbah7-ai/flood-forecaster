...
Serve policy (graceful degradation):
  ...
  * The app does NOT normally call this module's fetchers at all -- snapshot.py
    serves a scheduled snapshot, and these run only as a self-heal. When they do
    run, every fetcher degrades: fresh cache -> network -> LAST-GOOD cache (up to
    STALE_MAX_SECONDS, flagged `stale`) -> None/LiveDataError. Cache filenames
    carry no clock component, so a cached payload stays reachable the next day.
  * After a hard failure an endpoint is put on a short cooldown, because
    Streamlit reruns the script on every widget interaction and @st.cache_data
    does not cache exceptions -- without it, one 429 turned every slider drag
    into another request at the API that had just throttled us.
...
# How old a cached payload may be and still be served when the network fails.
# This is the "slightly stale beats a dead page" budget: 7 days is long enough to
# ride out a multi-day outage, and _staleness() will loudly flag the shifted
# window long before the data gets that old.
STALE_MAX_SECONDS = 7 * 24 * 3600
FAILURE_COOLDOWN_SECONDS = 900  # after a hard failure, don't re-hammer the API

USER_AGENT = ("flood-forecaster-daybyday/2.0 (USGS 01543000 research app; "
              "https://github.com/misbah7-ai/flood-forecaster)")
...
# ---------------------------------------------------------------------------
# 0. HTTP session + last-good disk cache  (graceful degradation, see snapshot.py)
# ---------------------------------------------------------------------------
def _build_session() -> requests.Session:
    """One pooled session with a real User-Agent and bounded retries.

    respect_retry_after_header is OFF on purpose: Open-Meteo can answer a 429
    with a Retry-After measured in minutes, and urllib3 would sleep for exactly
    that long -- hanging a page render. We prefer a short bounded backoff and
    then fall back to the last good cache."""
    s = requests.Session()
    ...
    except Exception:  # urllib3 API drift -> plain session, callers still degrade
        pass
    return s
...
def _cache_path(kind: str, *parts) -> Path:
    """Stable cache filename -- NO clock component.

    The previous key embedded '%Y%m%d%H', so a cached payload became unreachable
    the moment the hour ticked over and could never be reused the next day. TTL
    alone decides freshness now; the file itself persists as the last-good copy."""
    tag = "_".join(str(p) for p in parts if p not in (None, ""))
    return CACHE_DIR / f"{kind}_{tag}.json"


def _cache_read(path: Path, ttl: int = CACHE_TTL_SECONDS) -> tuple[dict | None, float | None, bool]:
    """(payload, age_seconds, is_fresh). Returns stale payloads too -- the caller
    decides whether to serve them after a failed fetch."""
    if not path.exists():
...
_FAILED_AT: dict[str, float] = {}  # in-process cooldown, keyed by cache filename


def _cooling_down(path: Path) -> bool:
    """True if this endpoint failed very recently.

    Streamlit re-runs the whole script on every widget interaction and
    @st.cache_data does NOT cache exceptions -- so without this, one 429 turned
    every slider drag into another request at the API that just throttled us."""
    t = _FAILED_AT.get(path.name)
    return t is not None and (time.time() - t) < FAILURE_COOLDOWN_SECONDS


def _mark_failed(path: Path) -> None:
    _FAILED_AT[path.name] = time.time()
...
def fetch_forecast_weather(lat: float, lon: float, use_cache: bool = True) -> pd.DataFrame:
    ...
    cache_file = _cache_path("forecast", f"{lat:.4f}", f"{lon:.4f}")
    cached, age, fresh = _cache_read(cache_file)
    if use_cache and fresh and cached:
        try:
            return _frame_from_payload(cached)
        except Exception:
            pass  # unreadable cache -> fall through to a fresh fetch

    def _serve_stale(reason: str) -> pd.DataFrame:
        """Last-good fallback: a forecast from a few hours ago is vastly more
        useful than a dead page, PROVIDED the age is carried through to the UI."""
        if cached is None or age is None or age > STALE_MAX_SECONDS:
            raise LiveDataError(reason)
        try:
            df = _frame_from_payload(cached)
        except Exception:
            raise LiveDataError(reason) from None
        df.attrs["stale"] = True
        df.attrs["stale_reason"] = reason
        df.attrs["age_seconds"] = age
        return df

    if _cooling_down(cache_file):
        return _serve_stale("Open-Meteo recently rate-limited us; not retrying yet")
    ...
    try:
        r = SESSION.get(FORECAST_URL, params=params, timeout=45)
        r.raise_for_status()
        payload = r.json()
    except requests.RequestException as e:
        _mark_failed(cache_file)
        return _serve_stale(f"Open-Meteo forecast request failed: {e}")
    except ValueError as e:
        _mark_failed(cache_file)
        return _serve_stale(f"Open-Meteo forecast returned malformed JSON: {e}")
    except Exception as e:  # noqa: BLE001 -- deliberately broad
        # The point of this function is that a fetch failure never kills the page.
        # Narrow excepts let odd transport errors (SSL, socket, urllib3 internals)
        # escape and defeat the whole fallback, so anything unexpected degrades
        # to the last-good copy too. _serve_stale still raises LiveDataError when
        # there is genuinely nothing cached to serve.
        _mark_failed(cache_file)
        return _serve_stale(f"Open-Meteo forecast request failed: {e!r}")
    if "daily" not in payload or "time" not in payload.get("daily", {}):
        _mark_failed(cache_file)
        return _serve_stale("Open-Meteo forecast response missing 'daily' data")

    _cache_write(cache_file, payload)
    _FAILED_AT.pop(cache_file.name, None)
...
def fetch_ensemble_weather(lat: float, lon: float, model: str = ENSEMBLE_MODEL,
    ...
    cache_file = _cache_path("ens", model, f"{lat:.4f}", f"{lon:.4f}")
    cached, age, fresh = _cache_read(cache_file)
    payload, is_stale = (cached, False) if (use_cache and fresh and cached) else (None, False)

    if payload is None and not _cooling_down(cache_file):
        try:
    ...
    if payload is None:  # network failed -> last-good band beats no band at all
        if cached is None or age is None or age > STALE_MAX_SECONDS:
            return None
        payload, is_stale = cached, True

    try:
        out = _ensemble_frame(payload)
        out["stale"] = is_stale
        if is_stale:
            out["age_seconds"] = age
        return out
...
def fetch_glofas(lat: float = 41.4133, lon: float = -78.1972,
    ...
    cache_file = _cache_path("glofas", f"{la:.4f}", f"{lo:.4f}")
    cached, age, fresh = _cache_read(cache_file)

    payload, is_stale = (cached, False) if (fresh and cached) else (None, False)
    if payload is None and not _cooling_down(cache_file):
        try:
            r = SESSION.get(GLOFAS_URL, params={
            ...
            if not payload.get("daily", {}).get("time"):
                raise ValueError("empty GloFAS daily block")
            _cache_write(cache_file, payload)
            _FAILED_AT.pop(cache_file.name, None)
        except Exception:
            _mark_failed(cache_file)
    ...
    if payload is None:  # last-good benchmark beats an empty comparison panel
        if cached is None or age is None or age > STALE_MAX_SECONDS:
            return None
        payload, is_stale = cached, True
...
def fetch_historical_forecast(lat: float, lon: float, start: str, end: str) -> pd.DataFrame | None:
    ...
    try:
        r = SESSION.get(HISTFC_URL, params={
            "latitude": lat, "longitude": lon,
            "daily": "precipitation_sum,temperature_2m_max,temperature_2m_min",
...
def fetch_archive_weather(lat: float, lon: float, start: str, end: str) -> pd.DataFrame | None:
    ...
    try:
        r = SESSION.get(ARCHIVE_URL, params={
            "latitude": lat, "longitude": lon,
            "daily": "precipitation_sum,temperature_2m_max,temperature_2m_min",
            "start_date": start, "end_date": end, "timezone": "America/New_York"}, timeout=60)
        r.raise_for_status()
...
def fetch_usgs_daily(gauge_id: str, start: str, end: str) -> pd.DataFrame | None:
    ...
    try:
        rows, offset = [], 0
        while True:
            r = SESSION.get(USGS_DAILY_URL, params={
